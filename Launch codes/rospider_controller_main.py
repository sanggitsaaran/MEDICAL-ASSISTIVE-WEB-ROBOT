...
#
from rospider_controller_interfaces import msg as jetmsg
from rospider_controller_interfaces.srv import SetPose1, SetPose1Request, SetPose1Response
from rospider_controller_interfaces.srv import SetPose2, SetPose2Request, SetPose2Response
from rospider_controller_interfaces.srv import PoseTransform, PoseTransformRequest, PoseTransformResponse
#
import rospider_sdk.buzzer as buzzer
from rospider_controller import rospider, build_in_pose, config
from rospider_controller.z_voltage_publisher import VoltagePublisher
from rospider_controller.z_joint_states_publisher import JointStatesPublisher
import geometry_msgs.msg


class rospiderControlNode:
    def __init__(self, node_name):
        ...
        self.controller = rospider.rospider(self)
        ...
        # 机器人姿态设置服务
        self.set_pose1_srv = rospy.Service("~set_pose_1", SetPose1, self.set_pose1_cb) # 通过姿态名称设置机器人的姿态， "DEFAULT_POSE" "DEFAULT_POSE_M"
        self.set_pose2_srv = rospy.Service("~set_pose_2", SetPose2, self.set_pose2_cb) # 通过六条腿的末端坐标设置机器人的姿态
        ...
        self.set_head_absolute = rospy.Subscriber("~set_head_absolute", jetmsg.TransformEuler, self.head_absolute_cb, queue_size=1)
        # 相对旋转
        self.set_head_relatively = rospy.Subscriber("~set_head_relatively", jetmsg.TransformEuler, self.head_relatively_cb, queue_size=1)
    ...
    def on_start(self, _):
        ...
        self.controller.set_joint(19, 0, 2)
        # 完毕
        buzzer.on()
        time.sleep(0.1)
        buzzer.off()
    ...
    def head_relatively_cb(self, msg: jetmsg.TransformEuler):
        yaw = msg.rotation.z
        pitch = msg.rotation.y
        old_yaw = self.controller.joints_state['camera_base_joint']  # 当前的关节角度
...
def main():
    rospider_controller_node = rospiderControlNode('rospider_control')
    try:
        rospy.spin()
    except Exception as e:
        rospy.logerr(str(e))
